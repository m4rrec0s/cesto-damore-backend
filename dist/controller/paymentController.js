...
class PaymentController {
    ...
    static async getPaymentStatus(req, res) {
        try {
            ...
            if (dbPayment.mercado_pago_id) {
                try {
                    mercadoPagoData = await paymentService_1.default.getPayment(dbPayment.mercado_pago_id);
                }
                catch (error) {
    ...
    static async handleWebhook(req, res) {
        try {
            ...
            const incomingType = webhookData?.type ||
                webhookData?.topic ||
    ...
    static async getFinancialSummary(req, res) {
        try {
            ...
            const isAdmin = req.user?.role === "admin"; // Assumindo role no user
            if (!isAdmin) {
                return res.status(403).json({
            ...
            const summary = await prisma_1.default.financialSummary.findMany({
                where: {
                    date: {
                        gte: start,
                        lte: end,
            ...
            const totals = summary.reduce((acc, day) => ({
                ...
                total_products_sold: acc.total_products_sold + day.total_products_sold,
                total_additionals_sold: acc.total_additionals_sold + day.total_additionals_sold,
            }), {
                total_sales: 0,
                total_net_revenue: 0,
                total_fees: 0,
    ...
    static async healthCheck(req, res) {
        try {
            const healthResult = await paymentService_1.default.healthCheck();
            res.json({
                success: true,
                data: healthResult,
