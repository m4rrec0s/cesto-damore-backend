...
const personalizationService_1 = __importDefault(require("../services/personalizationService"));
class PersonalizationController {
    ...
    async commit(req, res) {
        try {
            ...
            const result = await personalizationService_1.default.commitPersonalization(req.userId, {
                orderId,
                itemId,
                layoutBaseId,
    ...
    async preview(req, res) {
        try {
            return res.status(501).json({
                error: "Preview de composição não está mais disponível. Use upload direto.",
            });
        }
        catch (error) {
    ...
    async show(req, res) {
        try {
            const { id } = req.params;
            const personalization = await personalizationService_1.default.getById(id);
    ...
    async listByOrder(req, res) {
        try {
            ...
            const personalizations = await personalizationService_1.default.listByOrder(orderId, req.userId);
...
exports.default = new PersonalizationController();
