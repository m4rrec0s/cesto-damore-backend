...
const ruleTypeSchema = zod_1.z.enum([
    "PHOTO_UPLOAD",
    "LAYOUT_PRESET",
    "LAYOUT_WITH_PHOTOS",
    "TEXT_INPUT",
...
const productRuleInputSchema = zod_1.z.object({
    product_type_id: uuidSchema,
    rule_type: ruleTypeSchema,
    title: zod_1.z.string().min(1, "Título é obrigatório"),
    description: zod_1.z.string().optional(),
    required: zod_1.z.boolean().default(false),
    max_items: zod_1.z.number().int().positive().nullable().optional(),
    conflict_with: zod_1.z.array(zod_1.z.string()).nullable().optional(),
    dependencies: zod_1.z.array(zod_1.z.string()).nullable().optional(),
    available_options: zod_1.z.any().nullable().optional(),
    preview_image_url: zod_1.z.string().url().nullable().optional(),
...
class ProductRuleController {
    ...
    async createRule(req, res) {
        try {
            ...
            if (typeof parsedBody.available_options === "string") {
                try {
                    parsedBody.available_options = JSON.parse(parsedBody.available_options);
                }
                catch {
            ...
            if (req.file) {
                const customizationDir = path_1.default.join(process.cwd(), "images", "customizations");
            ...
            const rule = await prisma_1.default.productRule.create({
                data: {
                    ...
                    dependencies: payload.dependencies
                        ...
                        : null,
                    available_options: payload.available_options !== undefined
    ...
    async updateRule(req, res) {
        try {
            ...
            const existing = await prisma_1.default.productRule.findUnique({
                where: { id: ruleId },
            });
            if (!existing) {
                return res.status(404).json({
            ...
            if (req.file) {
                ...
                if (existing.preview_image_url &&
                    ...
                    try {
                        const oldFilename = existing.preview_image_url.replace("/images/customizations/", "");
                        const oldFilepath = path_1.default.join(customizationDir, oldFilename);
            ...
            const rule = await prisma_1.default.productRule.update({
                where: { id: ruleId },
                data: updateData,
            });
    ...
    async deleteRule(req, res) {
        try {
            ...
            await prisma_1.default.productRule.delete({
                where: { id: ruleId },
            });
            return res.json({
...
exports.default = new ProductRuleController();
