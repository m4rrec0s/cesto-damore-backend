...
class AIProductService {
    async getLightweightProducts(query) {
        try {
            // Extrair parâmetros de busca
            const { keywords, occasion, price_max, tag, available, q, } = query || {};
            ...
            const hasKeyword = (p, keywords) => {
                const text = (p.name +
                    " " +
                    (p.description || "") +
                    " " +
                    p.components.map((c) => c.item.name).join(" ")).toLowerCase();
                return keywords.some((k) => text.includes(k));
            };
            const premiumProducts = [];
            const standardProducts = [];
            const otherProducts = [];
            lightweightProducts.forEach((p) => {
                ...
                const text = (p.name + " " + p.description + " " + p.components.join(" ")).toLowerCase();
    ...
    async getProductDetail(idOrSlug) {
        try {
            ...
            return {
                ...
                final_price: this.calculateFinalPrice(product.price, product.discount || 0),
                ...
                optional_additionals: product.additionals.map((pa) => ({
                    ...
                    formatted_price: this.formatPrice(pa.custom_price || pa.additional.base_price),
                    image: pa.additional.image_url
                        ? this.formatImageUrl(pa.additional.image_url, baseUrl)
                        : null,
                    requires_photo: pa.additional.type === "quadro" ||
                        pa.additional.type === "polaroid" ||
                        pa.additional.name.toLowerCase().includes("foto"),
                    customization_type: pa.additional.customizations.length > 0
                        ? "configurable"
                        : "simple",
                })),
            };
    ...
    async searchProducts(query) {
        try {
            const { keywords, occasion, price_max, tag, has_custom_photo, available, q, } = query;
            const where = { is_active: true, type: { name: { equals: "Cestas", mode: "insensitive" } } };
            // Filtro de texto (keywords ou q)
            const searchTerm = keywords || q;
            if (searchTerm) {
                where.OR = [
    ...
    generateTags(product, occasions) {
        const tags = new Set();
        // Adiciona ocasiões como tags
        occasions.forEach((o) => tags.add(o
            .toLowerCase()
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .replace(/\s+/g, "-")));
        ...
        if (nameKeywords.includes("caneca"))
            tags.add("caneca");
        if (nameKeywords.includes("chocolate"))
            tags.add("chocolate");
        if (nameKeywords.includes("pelúcia") || nameKeywords.includes("pelucia"))
            tags.add("pelucia");
        if (nameKeywords.includes("quadro"))
            tags.add("quadro");
