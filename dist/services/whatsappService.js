...
class WhatsAppService {
    constructor() {
        ...
        this.ALERT_COOLDOWN = 3600000; // 1 hora em ms (evita spam)
        if (!process.env.EVOLUTION_API_URL ||
            !process.env.EVOLUTION_API_KEY ||
            !process.env.EVOLUTION_INSTANCE ||
            !process.env.WHATSAPP_GROUP_ID) {
            console.warn("⚠️ Variáveis de ambiente do WhatsApp não estão totalmente configuradas.");
        }
        this.config = {
    ...
    async sendMessage(text, phoneNumber) {
        if (!this.isConfigured()) {
            console.warn("WhatsApp não configurado. Configure as variáveis de ambiente.");
            return false;
        }
        try {
            const payload = {
    ...
    async checkAndNotifyLowStock(threshold = 5) {
        if (!this.isConfigured()) {
        ...
        try {
            ...
            for (const item of result.items) {
                try {
                    ...
                    if (item.current_stock === 0) {
                        ...
                        sent = await this.sendCriticalStockAlert(item.id, item.name, item.type, item.color_name
                            ? {
                        ...
                        // Estoque baixo
                        sent = await this.sendLowStockAlert(item.id, item.name, item.current_stock, item.threshold, item.type, item.color_name
                            ? {
                                name: item.color_name,
                                hex: item.color_hex_code || "",
                                additionalName: item.additional_name || "",
                    ...
                    console.error(`Erro ao enviar alerta para ${item.name}:`, error.message);
                    errors++;
                }
            }
            ...
            console.error("Erro ao verificar e notificar estoque baixo:", error.message);
    ...
    async sendStockSummary() {
        ...
        try {
            const report = await reportService_1.default.getStockReport(5);
            ...
            if (report.low_stock_items.length > 0) {
                ...
                const low = report.low_stock_items.filter((i) => i.current_stock > 0 && i.current_stock <= 2);
                const warning = report.low_stock_items.filter((i) => i.current_stock > 2);
    ...
    async sendOrderConfirmationNotification(orderData, options = {
        notifyTeam: true,
        ...
        try {
            const { teamMessage, customerMessage } = this.buildOrderConfirmationMessages(orderData);
            ...
            if (notifyCustomer) {
                ...
                const targetPhone = orderData.recipientPhone ?? orderData.customer.phone;
                if (targetPhone) {
                    ...
                    if (phoneWithCountry.length >= 12) {
                        customerSent = await this.sendDirectMessage(phoneWithCountry, customerMessage);
                    }
                    else {
                        console.warn(`Telefone inválido para notificação ao cliente: ${targetPhone}`);
                    }
                }
            }
    ...
    buildOrderConfirmationMessages(orderData) {
        const orderLabel = orderData.orderNumber || orderData.orderId.substring(0, 8).toUpperCase();
        ...
        if (orderData.paymentMethod) {
            teamMessage += `💳 Pagamento: ${this.formatPaymentMethod(orderData.paymentMethod)}\n\n`;
        }
        else {
            teamMessage += `\n`;
        }
        ...
        teamMessage += `• Telefone: ${orderData.customer.phone ?? "N/A"}${isAnonymous ? " (Envio anônimo)" : ""}\n`;
        if (orderData.delivery) {
            ...
            if (orderData.delivery.date) {
                teamMessage += `• Data: ${this.formatDateOnlyToBrasilia(orderData.delivery.date)}\n`;
            }
            if (complement) {
        ...
        if (orderData.delivery && orderData.delivery.date) {
            deliveryDateBrasilia = this.formatToBrasiliaTime(orderData.delivery.date);
        }
    ...
    formatOrderStatus(status) {
        const map = {
            PENDING: {
                ...
                customerHint: "Estamos aguardando a confirmação do pagamento para iniciar a preparação.",
            },
            PAID: {
                label: "Pagamento Confirmado",
                emoji: "✅",
                description: "Pagamento aprovado, preparar pedido.",
                customerHint: "Recebemos seu pagamento! Em breve começaremos a montar sua cesta.",
            },
            SHIPPED: {
                label: "Pedido em Rota de Entrega",
                emoji: "🚚",
                description: "Pedido saiu para entrega.",
                customerHint: "Seu pedido está a caminho! Avisaremos assim que a entrega for concluída.",
            },
            DELIVERED: {
                label: "Pedido Entregue",
                emoji: "🎁",
                description: "Pedido entregue ao cliente.",
                customerHint: "Pedido entregue com sucesso! Esperamos que tenha gostado da experiência.",
            },
            CANCELED: {
                label: "Pedido Cancelado",
                emoji: "❌",
                description: "Pedido cancelado.",
                customerHint: "O pedido foi cancelado. Caso tenha dúvidas, estamos à disposição para ajudar.",
            },
        };
        return map[status] || map.PENDING;
    }
    async sendOrderStatusUpdateNotification(orderData, newStatus, options = {}) {
