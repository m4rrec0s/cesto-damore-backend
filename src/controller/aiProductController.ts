...
class AIProductController {
  ...
  async searchProducts(req: Request, res: Response) {
    try {
      ...
      const result = await aiProductService.searchProducts(keywords);
  ...
  async getEndpointInfo(req: Request, res: Response) {
    res.json({
      ...
      description:
        "Endpoint otimizado para consultas de produtos por agentes de IA",
      usage: {
        without_keywords: {
          url: "/ai/products",
          description:
            "Retorna produtos na ordem de prioridade: premium caro > premium barato > caneca caro > outros barato",
        },
        with_keywords: {
          url: "/ai/products?keywords=aniversário romântico",
          description:
            "Busca contextual baseada em ocasiões, tipos de produto e faixas de preço",
        },
      },
      supported_keywords: {
        occasions: [
      ...
      response_structure: {
        _instructions:
          "Instruções gerais sobre prazos, adicionais e processo de compra",
        ...
        products: [
          {
            ...
            _ai_hints: {
              priority_category:
                "Categoria de prioridade do produto no catálogo",
