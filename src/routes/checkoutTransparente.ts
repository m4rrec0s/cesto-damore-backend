...
export const checkoutTransparenteRouter = Router();

/**
 * Serve a página HTML do Checkout Transparente
 */
checkoutTransparenteRouter.get(
  ...
  (req: Request, res: Response) => {
    try {
      ...
      const htmlPath = path.join(
        __dirname,
        "../pages/checkout-transparente.html"
      );
      ...
      if (publicKey) {
        htmlContent = htmlContent.replace(
          "TEST-8c043cc6-f2e4-4b8a-a282-dcdbcf0edcc9",
          publicKey.replace(/"/g, "") // Remove quotes se houver
        );
      }
 ...
 */
checkoutTransparenteRouter.post(
  "/payment/transparent",
  async (req: Request, res: Response) => {
    try {
      const {
      ...
      const paymentData = {
        ...
        paymentMethodId: payment_method_id as
          | "pix"
          | "credit_card"
          | "debit_card",
      ...
      if (payment_method_id === "pix" && paymentResult.raw) {
        const pixData = paymentResult.raw as any;
        ...
        response.data = {
          ...
          expires_at:
            pixTransactionData?.expiration_date ||
      ...
      if (
        ...
        errorMessage.includes("cc_rejected_bad_filled_security_code")
      ) {
        userFriendlyMessage =
  ...
  (req: Request, res: Response) => {
    ...
    <body>
      <div class="failure-container">
        ...
        <a href="/checkout-transparente?orderId=${
          orderId || ""
...
export default checkoutTransparenteRouter;
