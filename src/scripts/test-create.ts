...
async function main() {
  if (!process.env.DATABASE_URL) {
    console.error(
      "DATABASE_URL não está definido. Defina a variável de ambiente antes de executar os testes."
    );
    process.exit(1);
  }

  console.log(
    "Iniciando script de teste: criar productType, product, items, componente e adicional..."
  );
  ...
  const additionalItem = await prisma.item.create({
    data: {
  ...
  const pc = await prisma.productComponent.create({
    data: {
  ...
  const pa = await prisma.productAdditional.create({
    data: {
      product_id: product.id,
  ...
  console.log(
    ...
    JSON.stringify(productWithRelations, null, 2)
  );
